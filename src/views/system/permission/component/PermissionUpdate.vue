...
<script>
import {findPids, update} from "@/api/system/permission";
...
export default {
  ...
  mounted() {
    ...
    this.form.zindex = this.permission.zindex;
    this.form.menu = this.permission.menu;
  },
  methods: {
    findPids() {
      findPids(this.permission.id)
      .then(res => this.form.parent = res.data)
    },
    cancelHandle() {
    ...
    submitHandle() {
      this.$refs['form'].validate((valid) => {
        ...
        this.form.pid = pid
        update(this.permission.id,this.form)
        .then(() => {
